...
export type ApiResponse<T> = ( //copy from admin and replce with generic
  {
      status: 'success';
      data: T;
  } |
...
export function requestAdmins(callback: (response: ApiResponse<Admin[]>) => void) { //replace with new type and admin
  callback({
      status: 'success',
      data: admins
  });
...
export function requestUsers(callback: (response: ApiResponse<User[]>) => void) { //replace with new type and user
  callback({
      status: 'success',
      data: users
  });
...
export function requestCurrentServerTime(callback: (response: ApiResponse<number>) => void) { //new type + number
  callback({
      status: 'success',
      data: Date.now()
  });
...
export function requestCoffeeMachineQueueLength(callback: (response: ApiResponse<number>) => void) { //same
  callback({
      status: 'error',
